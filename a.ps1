...
Write-Host "Applying fixes to realtimenb..." -ForegroundColor Cyan

# 1. Update index.html to route all Live clicks to the correct Twitch Directory
$htmlPath = Join-Path (Get-Location) "public\index.html"
if (-not (Test-Path $htmlPath)) { $htmlPath = Join-Path (Get-Location) "index.html" }

if (Test-Path $htmlPath) {
    $html = Get-Content $htmlPath -Raw
    
    # Remove older versions of the script if they exist
    $html = $html -replace '(?s)<script id="twitch-injector">.*?</script>', ''
    
    # Using single quotes @'...'@ so PowerShell ignores $ inside the JS
    $scriptToInject = @'
    <script id="twitch-injector">
      document.addEventListener('click', (e) => {
        const isLiveList = e.target.closest('[data-live] > *');
        const isTrackerCard = e.target.closest('[data-score]');
        
        if (isLiveList || isTrackerCard) {
           const section = e.target.closest('[data-sport-card]');
           if (!section) return;
           
           const title = (section.querySelector('[data-title]')?.textContent || '').toLowerCase();
           let url = '';
           ...
           } else if (title.includes('cs2') || title.includes('csgo') || title.includes('counter-strike')) {
              url = 'https://www.twitch.tv/directory/category/counter-strike-2';
           }
...
'@
    ...
    Write-Host "[OK] Updated index.html Twitch click routing." -ForegroundColor Green
} else {
    Write-Host "[ERR] Could not find index.html" -ForegroundColor Red
}


# 2. Completely overwrite providers.js to fix LoL Kills, Valorant Riot API, and CS2 formatting.
...
if (Test-Path $providersPath) {
    # Using single quotes @'...'@ here is critical so JS template literals aren't parsed by PS
    $newProvidersJs = @'
// lib/providers.js
...
async function getNbaData() {
  ...
  return { games: result.games, upcoming: result.games.filter((g) => inNext12h(parseDate(g.startTime))), warning: null };
}
...
async function getLolData() {
  ...
  return { games: deduped, upcoming: deduped.filter((g) => inNext12h(parseDate(g.startTime))), warning: null };
}
...
async function getCsData() {
  for (const url of CS_SOURCES) {
    try {
      ...
      if (games.length) return { games, upcoming: games.filter((g) => inNext12h(parseDate(g.startTime))), warning: null };
    } catch {}
  }
...
'@
    ...
    Set-Content -Path $providersPath -Value $newProvidersJs -Encoding UTF8
    Write-Host "[OK] Rewrote providers.js with Riot APIs and formatting fixes." -ForegroundColor Green
} else {
    Write-Host "[ERR] Could not find providers.js" -ForegroundColor Red
}
