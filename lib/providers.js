...
<<<<<<< codex/build-real-time-nba-score-website-cap3c1
...
async function fetchLolRegionEvents(region, lolApiKey) {
  ...
  while (tokenQueue.length && seenTokens.size <= LOL_PAGE_LIMIT) {
    ...
    const payload = await fetchJson(`${LOL_BASE_URL}&leagueId=${encodeURIComponent(region.leagueId)}${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ''}`, {
