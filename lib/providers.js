...
const LOL_API_KEY_OVERRIDE = process.env.LOL_API_KEY || '';
const LOL_REGIONS = [
...
async function getLolApiKey() {
  if (LOL_API_KEY_OVERRIDE) return LOL_API_KEY_OVERRIDE;
  ...
  try {
    ...
    const key =
      html.match(/"apiKey":"([^"]+)"/)?.[1]
      || html.match(/"ESPORTS_API_KEY":"([^"]+)"/)?.[1]
      || html.match(/esportsApiKey"\s*:\s*"([^"]+)"/)?.[1]
      || html.match(/x-api-key"\s*:\s*"([^"]+)"/)?.[1]
      || null;
    if (!key) return null;
...
function parseEventStartTime(item) {
  ...
  if (typeof direct === 'string' && direct.trim()) {
    ...
    if (hhmm) {
      ...
      candidate.setUTCHours(Number(hhmm[1]), Number(hhmm[2]), 0, 0);
      if (candidate.getTime() + 6 * 3600000 < now.getTime()) {
...
async function getLolData() {
  ...
  if (!deduped.length) {
    const games = [
      serializeEsport({ matchId: 'demo-LoL-LCK-live-1', label: 'DNS vs DK', status: 'Live (fallback while upstream unavailable)', startTime: new Date(Date.now() - 15 * 60 * 1000).toISOString(), league: 'LoL LCK', streamUrl: LOL_TWITCH_BY_REGION.LCK }),
      serializeEsport({ matchId: 'demo-LoL-LEC-1', label: 'NAVI vs MKOI', status: 'Demo fallback (upstream unavailable)', startTime: futureIso(0.3), league: 'LoL LEC', streamUrl: LOL_TWITCH_BY_REGION.LEC }),
      serializeEsport({ matchId: 'demo-LoL-LEC-2', label: 'KC vs G2', status: 'Demo fallback (upstream unavailable)', startTime: futureIso(2), league: 'LoL LEC', streamUrl: LOL_TWITCH_BY_REGION.LEC }),
      serializeEsport({ matchId: 'demo-LoL-LCS-1', label: 'Team Liquid vs DSG', status: 'Demo fallback (upstream unavailable)', startTime: futureIso(5), league: 'LoL LCS', streamUrl: LOL_TWITCH_BY_REGION.LCS }),
      serializeEsport({ matchId: 'demo-LoL-LPL-1', label: 'BLG vs TES', status: 'Demo fallback (upstream unavailable)', startTime: futureIso(7), league: 'LoL LPL', streamUrl: LOL_TWITCH_BY_REGION.LPL })
    ];
