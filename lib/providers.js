...
async function getLolData() {
  ...
  const requests = LOL_REGIONS.map(async (region) => {
    const url = `${LOL_BASE_URL}&leagueId=${encodeURIComponent(region.leagueId)}`;
    ...
    return events.map((e) => {
      ...
      return serializeEsport({
        ...
        league: e.league?.name || `LoL ${region.key}`
      });
    });
  });
  ...
  if (!deduped.length) {
    const games = [
      ...fallbackEsports('LoL LCS', 'Cloud9', 'Team Liquid'),
      ...fallbackEsports('LoL LEC', 'G2', 'Fnatic'),
      ...fallbackEsports('LoL LPL', 'BLG', 'TES'),
      ...fallbackEsports('LoL LCK', 'T1', 'Gen.G')
    ];
    return { games, upcoming: games, warning: `LoL live feed unavailable: ${warnings.join(' | ') || 'unreachable'}` };
  }

  const warning = warnings.length ? `Some LoL regions unavailable: ${warnings.join(' | ')}` : null;
