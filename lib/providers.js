...
const fallbackNbaGames = () => [
  {
    gameId: 'demo-nba-1', label: 'BOS @ LAL', status: 'Demo fallback (upstream unavailable)', clock: 'Q1 • 12:00',
    home: { code: 'LAL', name: 'Lakers', city: 'Los Angeles', score: 0 }, away: { code: 'BOS', name: 'Celtics', city: 'Boston', score: 0 },
    startTime: futureIso(2), lastUpdated: new Date().toISOString()
  },
  {
    gameId: 'demo-nba-2', label: 'GSW @ DEN', status: 'Demo fallback (upstream unavailable)', clock: 'Q1 • 12:00',
    home: { code: 'DEN', name: 'Nuggets', city: 'Denver', score: 0 }, away: { code: 'GSW', name: 'Warriors', city: 'Golden State', score: 0 },
    startTime: futureIso(4), lastUpdated: new Date().toISOString()
  }
];
...
const pickByQuery = (events, query) => {
  const q = normalize(query);
...
async function getNbaData() {
  ...
  try {
    ...
    return { games, upcoming: games.filter((g) => inNext12Hours(parseDate(g.startTime))), warning: errors.join(' | ') };
  } catch (err) { errors.push(err.message); }

  const games = fallbackNbaGames();
  return { games, upcoming: games.filter((g) => inNext12Hours(parseDate(g.startTime))), warning: `Live NBA feeds unavailable: ${errors.join(' | ')}` };
}
...
async function getCsData() {
  try {
    ...
    const games = items.map((item) => serializeEsport({ matchId: item.id, label: `${item.team1?.name || item.team1 || 'TBD'} vs ${item.team2?.name || item.team2 || 'TBD'}`, status: item.status || 'scheduled', startTime: item.date || item.time || null, league: item.event?.name || item.tournament || 'Counter-Strike' }));
    return { games, upcoming: games.filter((g) => inNext12Hours(parseDate(g.startTime))), warning: null };
  } catch (err) {
...
async function getValorantData() {
  try {
    ...
    const games = items.map((item) => serializeEsport({ matchId: item.match_page || item.id, label: `${item.team1 || 'TBD'} vs ${item.team2 || 'TBD'}`, status: item.status || 'upcoming', startTime: item.unix_timestamp ? new Date(Number(item.unix_timestamp) * 1000).toISOString() : null, league: item.tournament_name || 'VALORANT', score: item.score || null }));
    return { games, upcoming: games.filter((g) => inNext12Hours(parseDate(g.startTime))), warning: null };
  } catch (err) {
