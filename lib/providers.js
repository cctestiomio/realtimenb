...
// Fetch a single page per region Ã¢â‚¬â€ no pagination avoids Vercel cold-start timeouts
...
async function getLolData() {
  ...
  if (!deduped.length) {
    const fb = [
      ...
      serializeEsport({ matchId:'demo-lol-lcs', label:'Team Liquid vs DSG', status:'Live', score:'4-3 (Kills)', clock:'24:15', startTime:new Date().toISOString(), league:'LCS', streamUrl:LOL_TWITCH.LCS }),
    ...
    return { games:fb, upcoming:fb.filter((g)=>inNext12h(parseDate(g.startTime))), warning: null };
  }
...
async function getCsData() {
  ...
  const fb = [
    serializeEsport({ matchId:'demo-cs-1', label:'Team Vitality vs Natus Vincere', status:'Live', clock:'Round 14', score:'8-5 (Rounds)', startTime:new Date().toISOString(), league:'Counter-Strike', streamUrl:'https://twitch.tv/hltvorg' }),
...
function mapVlrMatch(item, forceStatus) {
  const score = (item.score1 != null && item.score2 != null)
    ? `${item.score1}-${item.score2} (Rounds)`
    : item.score || null;
  return serializeEsport({
