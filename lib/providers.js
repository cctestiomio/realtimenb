...
const formatClock = (period, clock, fallback) => {
  ...
  // If timeStr is empty, use fallback only if it's not a live game scenario where clock might be empty (e.g. halftime)
  // But usually fallback is gameEt (start time), which we don't want in clock for live games.
  return `${period > 4 ? `OT${period - 4}` : `Q${period}`} \u2022 ${timeStr || '0:00'}`;
};

function serializeNbaFromCdn(game) {
  // 1=Scheduled, 2=InProgress, 3=Final
  ...
  if (sCode === 3) {
      ...
      if (/Halftime|Half/i.test(text)) {
          ...
          // If text contains End of Q..., use it, otherwise use clock
          if (/End\s+of/i.test(text)) clock = text;
          else clock = formatClock(game.period, game.gameClock, game.gameEt);
      }
  } else {
      // Scheduled
      status = 'Scheduled'; // Force scheduled to hide ET times
      clock  = '';
  }
