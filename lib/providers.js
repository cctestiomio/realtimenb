  ...
  try {
    ...
    for (const ev of p?.data?.schedule?.events || []) {
      if (isLiveStr(ev.state)) {
        liveEvents.push({ id: ev.id, startTime: parseDate(ev.startTime), leagueId: ev.league?.id });
      }
    }
    return liveEvents;
  } catch { return []; }
...
async function getLolData() {
  ...
  for (const result of settled) {
    ...
    for (const e of result.value.events) {
      ...
      if (isLive) {
        const activeGame = (e.match?.games || []).find(g => g.state !== 'completed');
        if (!activeGame) {
          gameTime = "[DEBUG] Waiting for Riot API game data...";
