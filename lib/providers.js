...
async function fetchLolRegionEvents(region, lolApiKey) {
  ...
  for (let page = 0; page < 4; page += 1) {
    const params = new URLSearchParams({ hl: 'en-US', leagueId: region.leagueId });
    if (pageToken) params.set('pageToken', pageToken);

    const payload = await fetchJson(`https://esports-api.lolesports.com/persisted/gw/getSchedule?${params.toString()}`, {
      headers: lolApiKey ? { 'x-api-key': lolApiKey } : {}
    });

    const batch = payload?.data?.schedule?.events || [];
    ...
    const next = payload?.data?.schedule?.pages?.newer || '';
...
async function getLolData() {
  ...
  const requests = LOL_REGIONS.map(async (region) => {
    const events = await fetchLolRegionEvents(region, lolApiKey);
    return events.map((e) => {
