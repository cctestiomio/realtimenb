...
function formatNbaTime(str) {
  ...
  // 1. Check NBA CDN format (e.g., "PT04M43.00S")
  const m = str.match(/PT(\d+)M(\d+)(?:\.(\d+))?S/);
  if (m) {
    const [, min, sec, ms] = m;
    const minStr = String(min).padStart(2, '0');
    const secStr = String(sec).padStart(2, '0');
    // Only show decimals if non-zero - prevents "04:43.00" from CDN source
    if (ms && ms.replace(/0/g, '') !== '') return `${minStr}:${secStr}.${ms.slice(0, 2)}`;
    return `${minStr}:${secStr}`;
  }
  
  // 2. Check ESPN format which sends plain "4:43" or "10:15"
  const espnMatch = str.trim().match(/^(\d+):(\d+)$/);
  if (espnMatch) {
    const minStr = String(espnMatch[1]).padStart(2, '0');
    const secStr = String(espnMatch[2]).padStart(2, '0');
    return `${minStr}:${secStr}`;
  }
...
const formatClock = (period, clock) => {
  ...
  // Also updated the default fallback here from '0:00' to '00:00'
  return `${period > 4 ? `OT${period - 4}` : `Q${period}`} ${timeStr || '00:00'}`;
};
